...
function resultProc(handle) {
    ...
    return (req, res, next) => tmp.reduce((s, e) => s.then(() => e({ params: req.params, body: req.body })), Promise.resolve())
...
function configRouter(bucketPath, resPath, bucketMeta, resMeta, props, parentProps, router) {
    router.get(bucketPath, resultProc(parentProps.existRes), (req, res) => res.send(bucketMeta))
        .post(resultProc(props.createHandle), (req, res) => res.status(HttpStatus.CREATED).send(getMeta(Object.assign({
            [getIdName(props.name)]: req.retVal
        }, req.params), resMeta)))
    router.get(resPath, resultProc(props.existRes), (req, res) => res.send(getMeta(req.paramsresMeta)))
        .put(resultProc([props.existRes, props.putHandle]))
}

function makeRouterBuilder(router) {
    return function(bucketPath, resPath, props, parentProps) {
        const bucketMeta = { url: bucketPath }
        const resMeta = { rul: resPath }
